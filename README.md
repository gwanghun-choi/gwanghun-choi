![header](https://capsule-render.vercel.app/api?type=waving\&color=0:0F172A,100:2563EB\&height=260\&section=header\&text=Gwanghun%20Choi\&fontSize=65\&fontColor=FFFFFF\&animation=fadeIn\&fontAlignY=38\&desc=Backend%20Developer%20%7C%20Java%20Spring%20%7C%20AI%20Automation\&descAlignY=58\&descAlign=50)
...
| ----------------- | ------------ | ----------------- | ---------------------------------------------------------------------------------- |
| 2024.10 ~ Present | 디딤365㈜ / 디딤㈜ | Backend Developer | Java Spring 기반 그룹웨어, HR, 공공기관 SI, AI Service Platform, RAG/Indexing |
...
<p> 
  ...
  <img src="https://img.shields.io/badge/AI Model-7C3AED?style=flat-square"/> 
...
공개 GitHub Repository에서 AI Agent Artifact를 자동으로 발견하고, 실제 선언 파일을 읽어 검색·탐색할 수 있도록 제공하는 Self-hosted Agent Artifact Index

Repository: [agentdock](https://github.com/gwanghun-choi/agentdock)

주요 내용

* Agent Skill, Claude Code Plugin, Plugin Marketplace, MCP Server, Slash Command, Hook Configuration 등 6종의 AI Agent Artifact 자동 탐지
* Repository 이름이나 Topic이 아닌 실제 파일 구조와 선언 형식을 기준으로 Artifact 식별
* Public, GitHub Stars 50 이상, Non-Fork, Non-Archived 조건과 실제 Artifact 존재 여부를 이용한 자동 Discovery Gate 구성
* MCP Registry, Seed List, Curated List, GitHub Topic Search를 통한 Repository Discovery 및 Request Budget 기반 수집 파이프라인 구현
* Git Commit SHA를 기준으로 Artifact가 읽힌 정확한 시점과 원본 파일을 추적할 수 있는 Provenance 구조 적용
* 마지막 수집 Commit과 현재 Commit이 동일한 Repository는 파일을 읽지 않고 종료하는 Incremental Refresh 적용
* Web Server와 Ingestion을 분리하고 하루 2회 Scheduled Sync로 Repository Discovery 및 Refresh 수행
* PostgreSQL Full Text Search와 Trigram 기반 유사 검색, Artifact Type 및 Capability Filter를 이용한 검색 기능 구현
* Artifact를 실행·미러링·추천하거나 안전성을 평가하지 않고 실제 파일의 선언 내용을 읽고 보여주는 Read-only Index 원칙 적용
* Next.js, React, TypeScript, PostgreSQL, Drizzle ORM 기반 Web Application 구성
...
* Google Drive API 연동을 통한 이력서 업로드 및 완료/실패 분류
...
* Python/FastAPI 기반 AI 이력서 분석 PoC 개발
* LLM 기반 채용공고/JD 매칭 및 후보자 평가 흐름 구현
* RAG 문서 Indexing 파이프라인 분석
* 문서 파싱, 청킹, 임베딩, Milvus Vector DB, OpenSearch 기반 검색 구조 분석
* Celery, RabbitMQ, Redis 기반 비동기 Worker 안정화 방향 검토

### Backend / Business System

* Java Spring 기반 그룹웨어, HR, ERP, 공공기관 업무시스템 RESTful API 개발
* 사용자, 조직, 권한, 인사, 파일, 메시징 등 주요 업무 도메인 개발
* 기존 .NET 시스템의 Java Spring 전환 및 데이터 마이그레이션 수행
* 외부 시스템 연동 API 및 중개 서버 개발
* 인증/인가, 공통 라이브러리, Jenkins 운영 관리 기능 개발

### Async / Batch / Messaging

* RabbitMQ 기반 메시지 처리 구조 개발
* Redis와 Scheduler 기반 배치성 데이터 처리
* 알림톡, 문자 발송, 주문/ERP 연동 등 외부 연동 실패 가능성이 있는 기능의 비동기 처리 경험
* 요청 처리와 백그라운드 작업 분리를 통한 응답 지연 및 장애 전파 감소
